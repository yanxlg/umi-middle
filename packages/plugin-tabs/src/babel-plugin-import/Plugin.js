...
export default class Plugin {
  ...
  includeFile(filepath){
    ...
    const pathList = Array.isArray(this.include)?this.include.map(file=>isAbsolute(file)?file:join(process.cwd(),file)):
      isAbsolute(this.include)?this.include:join(process.cwd(),this.include);
    if(Array.isArray(pathList)){
      return pathList.find((include)=>filepath.indexOf(include) ===0)
    }
    return filepath.indexOf(pathList) ===0
  }
  importMethod(methodName, file, pluginState) {
    if (!pluginState.selectedMethods[methodName]) {
      ...
      const targetMethodName = this.customComponentName?this.customComponentName(methodName):methodName;

      pluginState.selectedMethods[methodName] = this.transformToDefaultImport // eslint-disable-line
        ? addDefault(file.path, path, { nameHint: targetMethodName })
        : addNamed(file.path, targetMethodName, path);
