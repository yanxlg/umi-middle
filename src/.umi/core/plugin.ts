// @ts-nocheck
// This file is generated by Umi automatically
// DO NOT CHANGE IT MANUALLY!
import * as Plugin_0 from '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/app.tsx';
import * as Plugin_1 from '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-watermark/runtime.tsx';
import * as Plugin_2 from '@@/plugin-tabs/runtime';
import * as Plugin_3 from '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-sentry/runtime.tsx';
import * as Plugin_4 from '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-sauron/runtime.tsx';
import * as Plugin_5 from '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-history-inject/runtime.tsx';
import * as Plugin_6 from '@@/core/helmet.ts';
import * as Plugin_7 from '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-clickToComponent/runtime.tsx';
import { PluginManager } from 'umi';

function __defaultExport (obj) {
  if (obj.default) {
    return typeof obj.default === 'function' ? obj.default() :  obj.default
  }
  return obj;
}
export function getPlugins() {
  return [
    {
      apply: __defaultExport(Plugin_0),
      path: process.env.NODE_ENV === 'production' ? void 0 : '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/app.tsx',
    },
    {
      apply: Plugin_1,
      path: process.env.NODE_ENV === 'production' ? void 0 : '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-watermark/runtime.tsx',
    },
    {
      apply: Plugin_2,
      path: process.env.NODE_ENV === 'production' ? void 0 : '@@/plugin-tabs/runtime',
    },
    {
      apply: Plugin_3,
      path: process.env.NODE_ENV === 'production' ? void 0 : '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-sentry/runtime.tsx',
    },
    {
      apply: Plugin_4,
      path: process.env.NODE_ENV === 'production' ? void 0 : '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-sauron/runtime.tsx',
    },
    {
      apply: Plugin_5,
      path: process.env.NODE_ENV === 'production' ? void 0 : '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-history-inject/runtime.tsx',
    },
    {
      apply: Plugin_6,
      path: process.env.NODE_ENV === 'production' ? void 0 : '@@/core/helmet.ts',
    },
    {
      apply: Plugin_7,
      path: process.env.NODE_ENV === 'production' ? void 0 : '/Users/yanxianliang/Desktop/yonghui/umi-middle/src/.umi/plugin-clickToComponent/runtime.tsx',
    },
  ];
}

export function getValidKeys() {
  return ['patchRoutes','patchClientRoutes','modifyContextOpts','modifyClientRenderOpts','rootContainer','innerProvider','i18nProvider','accessProvider','dataflowProvider','outerProvider','render','onRouteChange','antd','qiankun','watermark','sentry','sauron','history-inject',];
}

let pluginManager = null;

export function createPluginManager() {
  pluginManager = PluginManager.create({
    plugins: getPlugins(),
    validKeys: getValidKeys(),
  });


  return pluginManager;
}

export function getPluginManager() {
  return pluginManager;
}
